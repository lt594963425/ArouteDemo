...
import com.android.library.utils.ThreadUtil;
...
public class LocationAmpUtils implements GeocodeSearch.OnGeocodeSearchListener {
    ...
    public void initLocation() {
        //初始化client
        locationClient = new AMapLocationClient(UIUtils.getContext());
        ...
        mGeocoderSearch = new GeocodeSearch(UIUtils.getContext());
        ...
        public void onLocationChanged(AMapLocation location) {
            if (null != location) {
                ...
                if (location.getErrorCode() == 0) {
                    if (mLocationListener != null && !TextUtil.isEmpty(location.getDistrict())) {
            ...
            ThreadUtil.getInstance().execute(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        RegeocodeAddress result = mGeocoderSearch.getFromLocation(query);// 设置同步逆地理编码请求
    ...
    public LatLng latLngToGDLatLng(LatLng sourceLatLng) {
        CoordinateConverter converter = new CoordinateConverter(UIUtils.getContext());
     ...
     * @return
     */
    public String calculateLineDistance(LatLng latLng1, LatLng latLng2) {
        float distance = AMapUtils.calculateLineDistance(latLng1, latLng2) / 1000;
        return String.format("%.2f", distance);
    }

    public String getCurrentDiatance(LatLng latLng) {
        float distance = AMapUtils.calculateLineDistance(latLng, new LatLng(mLat, mLng)) / 1000;
        return String.format("%.2f", distance);
    }
}
