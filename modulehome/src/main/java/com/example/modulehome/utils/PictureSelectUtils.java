package com.example.modulehome.utils;

import android.support.v7.app.AppCompatActivity;


import com.example.modulebase.data.constant.LC;
...
public class PictureSelectUtils {
    ...
    public static void pictureSelectorFormPhoto(AppCompatActivity activity, int mode, int maxSelectNum,
                                                boolean enableCrop, int selectedMode, List<LocalMedia> list, int second,
                                                int resuestCode) {
        ...
        PictureSelector.create(activity)
                ...
                .setOutputCameraPath(LC.TAKEPHOTO_PATH)// 自定义拍照保存路径
                ...
                .videoMaxSecond(second + 1)//显示多少秒以内的视频or音频也可适用
                .videoMinSecond(0)//显示多少秒以内的视频or音频也可适用
                .recordVideoSecond(second)//录制视频秒数 默认60s
...
//                .cropWH()// 裁剪宽高比，设置如果大于图片本身宽高则无效
     ...
     * @param selectedMode
     */
    public static void pictureSelectorSingleFormPhoto(AppCompatActivity activity, int mode, boolean enableCrop, int selectedMode, int resuestCode) {
        // 进入相册 以下是例子：不需要的api可以不写
        PictureSelector.create(activity)
                .openGallery(mode)// 全部.PictureMimeType.ofAll()、图片.ofImage()、视频.ofVideo()、音频.ofAudio()
                .theme(R.style.picture_PWJ_style)// 主题样式设置 具体参考 values/styles   用法：R.style.picture.white.style
